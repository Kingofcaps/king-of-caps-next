...
function Input({ name, label, type = "text", autoComplete, value, onChange, required = false }: { name: string; label: string; type?: string; autoComplete?: string; value: string; onChange: (value: string) => void; required?: boolean }) { return <label className="block text-sm font-bold text-zinc-800"><span className="mb-2 block">{label}</span><input name={name} type={type} autoComplete={autoComplete} value={value} onChange={(event) => onChange(event.target.value)} required={required} className="w-full rounded-xl border border-[#d4d4d4] bg-white px-4 py-3 text-zinc-900 outline-none transition focus:border-[#c9a227] focus:ring-2 focus:ring-[#c9a227]/15" /></label>; }
