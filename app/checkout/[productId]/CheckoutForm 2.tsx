  ...
  return (
    <main className="min-h-screen bg-[#fafafa] text-zinc-900">
      ...
      <div ref={formRef} className="mx-auto grid max-w-6xl gap-8 px-5 py-8 sm:px-8 sm:py-12 lg:grid-cols-[1.1fr_0.9fr]">
        ...
        <aside className="order-1 h-fit rounded-3xl border border-[#e5e5e5] bg-white p-5 shadow-xl shadow-zinc-200/60 lg:sticky lg:top-6"><p className="text-sm font-bold tracking-[0.2em] text-[#c9a227]">RÉSUMÉ DE LA COMMANDE</p><div className="mt-5 flex gap-4"><div className="relative h-24 w-24 shrink-0 overflow-hidden rounded-xl bg-zinc-100"><Image src={product.image} alt={product.name} fill sizes="96px" className="object-cover" /></div><div><h2 className="font-black">{product.name}</h2><p className="mt-2 text-sm text-zinc-500">Prix unitaire</p><p className="font-bold text-[#c9a227]">{formatPrice(unitPrice)}</p></div></div><div className="mt-6 border-y border-[#e5e5e5] py-5"><p className="text-sm font-bold text-zinc-800">Quantité</p><div className="mt-3 inline-flex items-center rounded-xl border border-[#d4d4d4] bg-white"><button type="button" onClick={() => setQuantity((value) => Math.max(1, value - 1))} className="px-4 py-2 text-xl">−</button><span className="min-w-10 text-center font-black">{quantity}</span><button type="button" onClick={() => setQuantity((value) => Math.min(product.stockQuantity, value + 1))} className="px-4 py-2 text-xl">+</button></div></div><div className="mt-5 flex items-center justify-between"><span className="font-bold">Total</span><strong className="text-2xl text-[#c9a227]">{formatPrice(total)}</strong></div></aside>
      </div>
    </main>
  );
}
