...
export default function CartPage() {
  ...
  return (
    <main className="min-h-screen bg-[#fafafa] text-zinc-950">
      <header className="border-b border-[#e5e5e5] bg-white"><div className="mx-auto flex max-w-6xl items-center justify-between px-5 py-4 sm:px-8"><Link href="/" className="text-sm font-black tracking-[0.2em]">KING OF CAPS</Link><CartLink /></div></header>
        ...
        {!hydrated ? (
          ...
          <div className="mt-8 grid gap-8 lg:grid-cols-[1fr_360px] lg:items-start">
            ...
            <aside className="rounded-3xl border border-[#e5e5e5] bg-white p-5 shadow-lg shadow-zinc-200/40 lg:sticky lg:top-5">
              ...
              <dl className="mt-5 space-y-3 text-sm"><div className="flex justify-between gap-4"><dt className="text-zinc-500">Articles</dt><dd className="font-bold">{itemCount}</dd></div><div className="flex justify-between gap-4"><dt className="text-zinc-500">Sous-total</dt><dd className="font-bold">{formatDualPrice(subtotal)}</dd></div><div className="flex justify-between gap-4"><dt className="text-zinc-500">Livraison</dt><dd className="text-right font-semibold">Calculée au checkout</dd></div></dl>
              <div className="mt-5 flex justify-between border-t border-zinc-200 pt-5"><span className="font-black">Total provisoire</span><strong className="text-xl text-[#a8861e]">{formatDualPrice(subtotal)}</strong></div>
