...
create table if not exists public.boutique_sales (
  ...
  source_stock_movement_id uuid not null references public.stock_movements(id) on delete restrict,
  created_at timestamptz not null default now(),
  unique (source_stock_movement_id)
);

create index if not exists boutique_sales_product_id_idx
  on public.boutique_sales (product_id);

create index if not exists boutique_sales_sold_at_idx
  on public.boutique_sales (sold_at desc);

alter table public.boutique_sales enable row level security;
revoke all on table public.boutique_sales from anon, authenticated;
...
begin
  ...
  where movement.movement_type = 'decrease'
    ...
    and not exists (
      select 1
      from public.boutique_sales sale
      where sale.source_stock_movement_id = movement.id
    );
...
insert into public.boutique_sales (
  product_id,
  product_name,
  quantity,
  unit_price,
